...
import sys
sys.path.append(r'/protect_content/utils/delays.py')
from .protect_content.utils.delays import get_delays
...
def get_input_type_to_copy():
    ...
    print("9 - Polls\n")
    print(
        "Insira o(s) número(s) do tipo de arquivo a ser clonado, separando por vírgula."
    )
    print("Por exemplo, para copiar fotos e documentos digite: 1,3")
...
def task_type():
    ...
    print("Nova clonagem ou continuação?\n1 = new\n2 = resume")
    answer = input("Sua Resposta: ")
    if answer == "1":
        return 1
    elif answer == "2":
...
def check_chat_id(chat_id):
    ...
    try:
        ...
        if MODE == "bot":
            print(
                "\nVerifique se o bot faz parte do chat como administrador."
                + "It is necessary for bot mode."
            )
        else:
            print("\nVerifique se a conta do usuário faz parte do chat")
        return False
    ...
    if client_name == "user":
        if Path(f"{client_name}.session").exists():
            try:
                ...
                print("Exclua o arquivo de sessão e tente novamente.")

        while True:
            try:
                api_id = int(input("Insira seu api_id: "))
                api_hash = input("Insira seu api_hash: ")
    ...
    if client_name == "bot":
        ...
        while True:
            try:
                ...
                bot_token = input("Digite seu bot_token: ")
...
def main():
    ...
    print(
        "\nClonagem de bate-papo concluída! :)\n"
        + "Se você não vai continuar esta tarefa para estes chats, "
        + "exclua o arquivo posted.json"
    )
...
parser.add_argument(
    "--mode",
    choices=["user", "bot"],
    help='"user" is slow. "bot" requer token_bot em credenciais',
)
parser.add_argument(
...
def check_chat_id(chat_id):
    try:
        ...
        raise ValueError(f"ID do chat inválido: {chat_id}. {e}")
    except pyrogram.errors.exceptions.bad_request_400.PeerIdInvalid as e:
        raise ValueError(f"ID do chat inválido: {chat_id}. O bot pode não ter acesso a esse chat. {e}")
    except Exception as e:
        raise Exception(f"Erro inesperado ao verificar o chat: {e}")

try:
    origin_chat = int(origin_chat) #Transforma em inteiro
    ORIGIN_CHAT_TITLE = check_chat_id(origin_chat)
    # ... (seu código restante) ...
except ValueError as e:
    print(f"Erro: {e}")
